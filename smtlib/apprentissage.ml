...
let print_char_list l = List.iter (fun c -> printf "%c" c) l

(* ======================================================================= *)
(* Utils functions *)

let map_flatten_sort_uniq map_fun compare_fun l =
  List.map map_fun l |> List.flatten |> List.sort_uniq compare_fun
;;
...
let alphabet_from_list l =
  map_flatten_sort_uniq explode (fun a b -> int_of_char a - int_of_char b) l
;;

(* test *)
(* let () = *)
(*   print_string "alphabet test 1 = "; *)
(*   print_char_list (alphabet_from_list (li @ le)); *)
(*   print_string "\n" *)
(* ;; *)

(* let () = *)
(*   print_string "alphabet test 2 = "; *)
(*   print_char_list (alphabet_from_list ([ "e"; "vf"; "c" ] @ [ "vf"; "s"; "ab" ])); *)
(*   print_string "\n" *)
(* ;; *)
...
let rec prefixes s =
  let prefixes_list = ref [ "e" ] in
  iteri (fun i _ -> prefixes_list := !prefixes_list @ [ "e" ^ sub s 0 (i + 1) ]) s;
  !prefixes_list
;;
...
let prefixes_of_list l = map_flatten_sort_uniq prefixes compare l
...
let declare_types_alphabet cl =
  let alphabet_str =
    List.fold_left (fun alpha c -> alpha ^ " " ^ string_of_char c) "" cl
  in
  "(A" ^ alphabet_str ^ ")"
;;
...
let declare_types_trie l =
  let prefixes_str =
    List.fold_left (fun pref s -> pref ^ " " ^ s) "" (prefixes_of_list l)
  in
  "(T" ^ prefixes_str ^ ")"
;;
...
(* let () = *)
(*   let a = alphabet_from_list (li @ le) in *)
(*   Printf.printf "%s" (declare_types (li @ le) a) *)
(* ;; *)

(* let () = *)
(*   print_string "prefixes 'aba' = ["; *)
(*   prefixes_of_list (li @ le) |> List.iter (fun s -> print_string (" '" ^ s ^ "'")); *)
(*   print_endline " ]" *)
(* ;; *)

(* let () = li @ le |> alphabet_from_list |> declare_types_alphabet |> print_endline *)
...
let define_Q_set = "(define-sort Q () Int)\n(declare-const n Q)\n(assert (> n 0))\n\n"
...
let declare_final = "(declare-fun final (Q) Bool)\n\n"

let declare_f =
  "(declare-fun f (T) Q)\n(assert (forall ((x T))\n(and (>= (f x) 0) (< (f x) n))))\n\n"
;;

let assert_init = "(assert (= 0 (f e)))\n\n"

let define_sorts_and_functions () =
  define_Q_set ^ declare_delta ^ declare_final ^ declare_f ^ assert_init
;;

(* let () = print_string define_sorts_and_functions *)
...
let eq_trans_constr s a =
  "(= (f " ^ s ^ string_of_char a ^ ")  (delta (f " ^ s ^ ")  " ^ string_of_char a ^ "))"
;;
...
let rec list_transition_constraints l =
  List.map
    (fun sa ->
      ...
      if 0 != len
      then (
        let s = sub sa 0 (len - 1) in
        let a = get sa (len - 1) in
        eq_trans_constr s a)
      else sa)
    l
;;
...
let assert_transition_constraints l =
  "(assert (and\n            "
  ^ (prefixes_of_list l
    |> List.filter_map (fun s -> if equal "e" s then None else Some s)
    |> list_transition_constraints
    |> concat "\n\t    ")
  ^ "))\n\n"
;;
...
let assert_acceptance li le =
  "(assert (and "
  ^ String.concat "\n             " (List.map eq_accept li)
  ^ "\n             "
  ^ String.concat "\n             " (List.map eq_non_accept le)
  ^ "))\n\n"
;;
...
let read_lists_from_file (filename : string) : string list * string list =
  ...
  String.split_on_char ' ' (List.nth lines 0), String.split_on_char ' ' (List.nth lines 1)
;;
